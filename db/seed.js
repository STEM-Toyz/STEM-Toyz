...
const seedReviews = [
  ...
  { title: "Not safe for kids!", stars: 1, content: 'My 5 year old chopped my sofa in half with this light saber', product_id: 2, user_id: 2},
...
const seedOrders = [
  ...
  { status: 'shipped', user_id: 2, address_id: 3}
];
...
db.didSync
  ...
  .then(() => {
    return Promise.all([
      User.bulkCreate(seedUsers),
      Product.bulkCreate(seedProducts)
    ])
  })
  .then(() => {
    return Promise.all([
      Review.bulkCreate(seedReviews),
      Address.bulkCreate(seedAddresses)
    ])
  })
  .then(() => {
    return Order.bulkCreate(seedOrders)
  })
  .then(() => {
    return Item.bulkCreate(seedItems)
  })
  ...
  //   // .then(seedOrders)
  //   // .then(seedOrders => console.log(`Seeded ${seedOrders.length} Orders OK`))
  //   .then(seedItems)
  //   .then(items => console.log(`Seeded ${items.length} Items OK`))
  //   // .then(seedProducts)
  //   // .then(seedProducts => console.log(`Seeded ${seedProducts.length} Products OK`))
  //   // .then(seedReviews)
  //   // .then(seedReviews => console.log(`Seeded ${seedReviews.length} Reviews OK`))
  //   .catch(error => console.error(error))
  //   .finally(() => db.close())
  ...
  //   { quantity: 15, order_id: 2},
  //   { quantity: 30, order_id: 3}
  // ], item => db.model('items').create(item));
  //
  //
  // // const seedReviews = () => db.Promise.map([
  // //   { title: "Best Toy Ever!", stars: 5, content: 'I was so impressed with this product' },
  // //   { title: "Not safe for kids!", stars: 1, content: 'My 5 year old chopped my sofa in half with this light saber' },
  // //   { title: "pretty good", stars: 4, content: 'not bad, not bad at all' }
